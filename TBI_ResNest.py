...
pre_c2 = tf.keras.metrics.Precision(name='precision')
re_c2 = tf.keras.metrics.Recall(name='recall')
...
class ResNest:
    def __init__(self, height, width, channel, num_class,
        ...
        self.resModel = self.model(tf.zeros([1, self.height, self.width, self.channel]))
    ...
    def step(self, x, y, iteration=0, train=False, paths=None):
    ...
    def model(self, verbose=False, mode=0, paths=None):
        if mode == 0:
            ...
            path = paths[tmpidx]
            inputs = []
            while path == paths[tmpidx]:
                imp = tf.keras.layers.Input(shape=[self.height, self.width, self.channel])
                inputConv = tf.keras.layers.Conv2D(32, 3, stride=1, padding='SAME', name='inpConv_%d' % tmpidx)(imp)
                input_act = tf.keras.layers.ELU(name='inpConv_%d' % tmpidx)(inputConv)
...
class Dataset(object):
    ...
    def __init__(self, train_path=None, val_path=None, train_name=None, val_name=None, mode=0):

        print("\nInitializing Dataset...")
        train_data = np.load(train_path)
        val_data = np.load(val_path)
        training_paths = np.load(train_name)
        validation_paths = np.load(val_name)
        ...
        if mode == 0:
            x_tr = x_tr.reshape([-1, 256, 64, 15])
            x_te = x_te.reshape([-1, 256, 64, 15])
        else:
            x_tr = x_tr.reshape([-1, 256, 64, 3])
            x_te = x_te.reshape([-1, 256, 64, 3])
        ...
        self.num_tr, self.num_te = self.x_tr.shape[0], self.x_te.shape[0]
        self.idx_tr, self.idx_te = 0, 0
        self.tr_pa = training_paths
        self.te_pa = validation_paths
    ...
    def next_train(self, batch_size=1, fix=False, mode=0):

        if mode == 1:
            start = self.idx_tr
            tmpidx = 0
            while tmpidx != batch_size:
                pathName = self.tr_pa[self.idx_tr]
                self.idx_tr += 1
                while pathName == self.tr_pa[self.idx_tr]:
                    self.idx_tr += 1
        ...
        if x_tr.shape[0] != batch_size and mode == 0:
            x_tr, y_tr = self.x_tr[-1-batch_size:-1], self.y_tr[-1-batch_size:-1]

        return x_tr, self.label2vector(labels=y_tr), p_tr, terminator

    def next_test(self, batch_size=1, mode=0):

        if mode == 1:
            start = self.idx_te
            tmpidx = 0
            while tmpidx != batch_size:
                pathName = self.te_pa[self.idx_te]
                self.idx_te += 1
                while pathName == self.te_pa[self.idx_te]:
                    self.idx_te += 1
        ...
        if x_te.shape[0] != batch_size and mode == 0:
            x_te, y_te = self.x_te[-1-batch_size:-1], self.y_te[-1-batch_size:-1]
        return x_te, self.label2vector(labels=y_te), p_tr, terminator


def training(neuralnet, dataset, epochs, batch_size, mode=0, paths=None):
    ...
    for epoch in range(epochs):
        ...
        while True:
            if mode == 0:
                x_tr, y_tr, paths, terminator = dataset.next_train(batch_size)  # y_tr does not used in this prj.
                loss, accuracy, class_score = neuralnet.step(x=x_tr, y=y_tr, iteration=iteration, train=True,
                                                             paths=paths)
            else:
                x_tr, y_tr, paths, terminator = dataset.next_train(batch_size, mode=1)
                loss, accuracy, class_score = neuralnet.step(x=x_tr, y=y_tr, iteration=iteration, train=True,
                                                             mode=1, paths=paths)
...
def test(neuralnet, dataset, epoch):
    ...
    total_loss = 0
    while True:
        x_te, y_te, _, terminator = dataset.next_test(1)  # y_te does not used in this prj.
...
def main():
    ...
    train_data = '/data/TBI/Datasets/NPFiles/IPHAug/TrainingData.npy'
    train_path = '/data/TBI/Datasets/NPFiles/IPHAug/TrainingPaths.npy'
    val_data = '/data/TBI/Datasets/NPFiles/IPHAug/ValidationData.npy'
    val_path = '/data/TBI/Datasets/NPFiles/IPHAug/ValidationPaths.npy'
    dataset = Dataset(train_data, val_data, train_path, val_path)
    ...
    training(neuralnet=neuralnet, dataset=dataset, epochs=15, batch_size=64)
    # test(neuralnet=neuralnet, dataset=dataset)
    neuralnet.resModel.save('/data/TBI/Datasets/Models/ResNeSt_IPHAug')
